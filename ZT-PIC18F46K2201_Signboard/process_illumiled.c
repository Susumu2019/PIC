...
int illumi_putcount = 0;//変更時のみ送信する場合の送信カウント
int illumi_putcount_max = 5;//変更時のみ送信する場合の送信回数
...
void illumiled_process(){
	...
	if(mode == 700){
		...
		if(illumiled_process_one > Illumi_speed){
			...
			if(illumi_putcount < illumi_putcount_max){
				illumi_putcount++;
		...
		if(cursor_index == 1){
			...
			//up
			if(tmp_sw3_true != 0){tmp_sw3_true = 0;
				if(Illumi_timing >= 3){	Illumi_timing = 0;
				}else{					Illumi_timing++;	}
			...
			//down
			if(tmp_sw4_true != 0){tmp_sw4_true = 0;
				if(Illumi_timing == 0){	Illumi_timing = 3;
				}else{					Illumi_timing--;	}
				write_eeprom(0x14,Illumi_timing);//設置値を保存
			}
