...
void startup(){
	...
	if(mode == 0xffff){		mode = 0;}//未設定ならデフォルト値設定
	//イルミネーション
	Illumi_color_set = READ_EEPROM(0x10);	if(Illumi_color_set == 0xff){Illumi_color_set = 1;}
...
void main(){
	...
	while(true){
		...
		if(mode >= 0 && mode < 100){
			modeselect_process();
			if(lcd_process_one > 1){lcd_process_one = 0;		modeselect_lcd_process();}
		}else if(mode >= 100 && mode < 200){
			//ハードウェア表示
			hardwareinfo_process();
			if(lcd_process_one > 1){	lcd_process_one = 0;	hardwareinfo_lcd_process();}
		}else if(mode >= 200 && mode < 300){
			//入出力表示
			io_process();
			if(lcd_process_one > 1){	lcd_process_one = 0;	io_lcd_process();}
			...
			//シリアル表示
			uart_process();
			if(lcd_process_one > 1){	lcd_process_one = 0;	lcd_process();	}
		}else if(mode >= 400 && mode < 500){
			//グラフィック表示
			graphic_process();
			if(lcd_process_one > 1){	lcd_process_one = 0;	lcd_process();	}
		}else if(mode >= 500 && mode < 600){
			//時計処理
			rtc_process();
			if(lcd_process_one > 1){	lcd_process_one = 0;	rtc_lcd_process();	}
		}else if(mode >= 600 && mode < 700){
			//サーボ処理
			if(servo_process_one != 0){servo_process_one = 0;	servo_process();}  
			if(lcd_process_one > 1){	lcd_process_one = 0;	lcd_process();	}
		}else if(mode >= 700 && mode < 800){
			//イルミネーションLED処理
			illumiled_process();
			if(lcd_process_one > 1){	lcd_process_one = 0;	illumiled_lcd_process();}
		}else if(mode >= 800 && mode < 900){
			//Temp&RH 		
			sw_process();
			if(lcd_process_one > 1){	lcd_process_one = 0;	lcd_process();	}
		}else if(mode >= 900 && mode < 1000){
			//IR(赤外線)
			sw_process();
			if(lcd_process_one > 1){	lcd_process_one = 0;	lcd_process();	}
			...
			sw_process();
			if(lcd_process_one > 1){	lcd_process_one = 0;	lcd_process();	}
		}else if(mode >= 1100 && mode < 1200){
			//MP3
			sw_process();
			if(lcd_process_one > 1){	lcd_process_one = 0;	lcd_process();	}
		}else if(mode >= 1200 && mode < 1300){
			//Motion
			sw_process();
			if(lcd_process_one > 1){	lcd_process_one = 0;	lcd_process();	}
		}else{
...
void sw_process(){
	...
	if(tmp_sw2_true != 0){tmp_sw2_true = 0;
		if(mode == 800){		//Temp&RH
			mode = 10;	cursor_index = 8;
		}else if(mode == 900){	//IR
			mode = 10;	cursor_index = 9;
		}else if(mode == 1000){	//RFID
			mode = 10;	cursor_index = 10;
		}else if(mode == 1100){	//MP3
			mode = 10;	cursor_index = 11;
		}else if(mode == 1200){	//Motion 
			mode = 10;	cursor_index = 12;
		}
	}
...
void hardwareinfo_process(){
	...
	//if(tmp_sw3_true != 0){tmp_sw3_true = 0;
	//	//カーソルを上へ
	//	if(cursor_index > 1){	cursor_index--;	}
	//}
	////down
	//if(tmp_sw4_true != 0){tmp_sw4_true = 0;
	//	//カーソルを下へ
	//	if(cursor_index < 5){	cursor_index++;	}
	//}
}
...
void servo_process(){
	...
	if(tmp_sw1_true != 0){tmp_sw1_true = 0;
		//カーソルを下へ
		if(cursor_index < 5){	cursor_index++;	}else{cursor_index = 1;}
	}
	//cansel
	if(tmp_sw2_true != 0){tmp_sw2_true = 0;
		mode = 10;	
	...
	if(tmp_sw3_true != 0){tmp_sw3_true = 0;
		...
		if(cursor_index == 1){
			...
			if(servo_target_angle < 180){servo_target_angle++;}
		}else if(cursor_index == 3){
			if(servo_step_angle < 180){servo_step_angle++;}
			...
			if(servo_turn == 0){
				...
				servo_start = 0;servo_rotation = 0;
			}
		}
	}
	//down
	if(tmp_sw4_true != 0){tmp_sw4_true = 0;
		...
		if(cursor_index == 1){
			...
			if(servo_target_angle > 0){servo_target_angle--;}
		}else if(cursor_index == 3){
			if(servo_step_angle > 0){servo_step_angle--;}
		}else if(cursor_index == 4){
			servo_turn = 0;
		}else if(cursor_index == 5){
			if(servo_turn == 0){
				...
				servo_start = 1;servo_rotation = 0;
			}
		}
	}
	...
	if(cursor_index == 5){
		...
		if(servo_turn == 1 && servo_start == 0){
		}else{
			if(servo_position == 0){//原点目標
				if(servo_angle < servo_step_angle){		servo_angle = 0;
				}else{		servo_angle = servo_angle - servo_step_angle;	}
			}else{//ターゲット角目標
				if((servo_target_angle-servo_angle) < servo_step_angle){		servo_angle = servo_target_angle;
				}else{		servo_angle = servo_angle + servo_step_angle;	}
			}
		}
	...
	//サーボ用PWM信号
	//0.5msec～2.4msec 20msec周期推奨
	//1900msec÷180度=10.56
	servo_time = (servo_angle*10.56)+500;
	ra7 = 1;	output_bit(PIN_A7,ra7);	
	delay_us(servo_time);
	ra7 = 0;	output_bit(PIN_A7,ra7);

	//analog_value = 0;//0-1024
...
//	//char *param1;
//
//	while(kbhit(c2)){  // マイコンの受信バッファ内のデータを全て受信する
//		get_command_c2[get_command_value_c2] = fgetc(c2);//受信データ取り出し
//		fprintf(c2,"%c",get_command_c2[get_command_value_c2]);
//		get_command_value_c2++;
//	}
//
//	if(get_command_c2[get_command_value_c2-1]==0xd){
//		//コマンドとパラメータを分割
//		command = strtok(get_command_c2,punctuate);
//		//param1 = atoi(strtok(null,punctuate));
//		param1 = strtok(null,punctuate);
//
//		fprintf(c2,"\r\n");
//
//		strcpy(compare_command_c2,"?\r");
//		if(strcmp(command,compare_command_c2)==0){
//			//ヘルプ
//			fprintf(c2,"ZT-****-SOFT Ver.0.90 UART2\r\n");
//			//rtc_date_read();			
//			//fprintf(c2,"%02x/%02x/%02x %02x:%02x:%02x\r\n",year,month,day,hour,min,sec);
//		}
//
//		//strcpy(compare_command_c2,"time");
//		//if(strcmp(command,compare_command_c2)==0){
//		//	//時間を設定
//		//	//設定例：time,101006101840(2010年10月06日10時18分40秒)
//		//	
//		//	if(strlen(param1) == 13){
//		//		//パラメータを分割
//		//		year = ((param1[0] - 0x30)*16) + (param1[1] - 0x30);
//		//		month = ((param1[2] - 0x30)*16) + (param1[3] - 0x30);
//		//		day = ((param1[4] - 0x30)*16) + (param1[5] - 0x30);
//		//		hour = ((param1[6] - 0x30)*16) + (param1[7] - 0x30);
//		//		min = ((param1[8] - 0x30)*16) + (param1[9] - 0x30);
//		//		sec = ((param1[10] - 0x30)*16) + (param1[11] - 0x30);
//
//		//		rtc_date_set();
//		//		fprintf(c2,"time set ok.\r\n");
//		//	}else{
//		//		fprintf(c2,"Parameter error.\r\n");
//		//	}
//
//		//}
//
//		//strcpy(compare_command_c2,"time\r");
//		//if(strcmp(command,compare_command_c2)==0){
//		//	//現在時刻を表示
//		//	rtc_date_read();			
//		//	fprintf(c2,"%02x/%02x/%02x %02x:%02x:%02x\r\n",year,month,day,hour,min,sec);
...
//			get_command_c2[tmp1] = 0x0;
//		}
//		get_command_value_c2 = 0;
//
//		//参照用のコマンドもクリア
//		for(tmp1=0;tmp1<20;++tmp1){
//			compare_command_c2[tmp1] = 0x0;
//		}
//	}
//}
